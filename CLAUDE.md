...
- 檔名格式：`northstar.png`, `houdini.png`, `watchdog.png`, 等等（小寫、無空格）
...
**Vampire** - Data Analyst
- **Avatar**：`assets/avatars/vampire.png`
- **一句話**：`我從數據裡吸取真相。`
...
- [ ] Vampire Avatar：`assets/avatars/vampire.png`
...
這個專案已有上一版的 code（Next.js + Tailwind + next-intl + next-themes）。請基於現有 code 更新，不需要從零開始。

**主要更新**：
1. 確保所有內容與此 spec 一致
2. Avatar 暫時用漸層色塊 + Lucide icon 佔位（之後換成真實插圖）
3. 確保 Marawanna 的 App Store 連結正確
4. Products Section 要有 2 個佔位產品
5. 確保雙語翻譯完整

**完成後（必須執行）**：
1. `pnpm build` — 失敗就修到通過
2. `git add -A && git commit -m "feat: update to spec v1.3-with-assets"`
3. `git push origin main`
4. `openclaw system event --text "Done: zone lab website updated to spec v1.3-with-assets — 已 push" --mode now`
